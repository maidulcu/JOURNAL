...
// Add meta boxes
require_once('library/meta-box/meta-box.php' ); 

// Add meta boxes required file
require_once('library/meta-box/init.php' ); 
...
function bones_custom_admin_footer() {
	echo '<span id="footer-thankyou">Developed by <a href="http://dynamicweblab.com" target="_blank">Dynamicweblab.com</a></span> with help of <a href="http://themble.com/bones" target="_blank">Bones</a>.';
}
...
/* ---------------------------------------------------------------------- */
/*	Breadcrumb https://gist.github.com/4032941
/* ---------------------------------------------------------------------- */
...
/**
 * Check if post have video for single post
 *
 */
 
function get_post_video(){
    
		$post_video_type = get_post_meta(get_the_ID(), '_cmb_post_video_select', true); 
        $post_video_id = get_post_meta(get_the_ID(), '_cmb_post_video_id', true);
        
        if($post_video_id != 0){ 

            if($post_video_type == 'Youtube') { 
                echo '<iframe width="99%" height="350" src="http://www.youtube.com/embed/'.$post_video_id.'?rel=0" frameborder="0" allowfullscreen></iframe>';
            }
            elseif($post_video_type == 'Vimeo') { 
                echo '<iframe src="http://player.vimeo.com/video/'.$post_video_id.'?title=0&amp;byline=0&amp;portrait=0&amp;color=ba0d16" width="100%" height="350" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>';		
            }
        }			
...
function get_wpbs_theme_options(){
  $theme_options_styles = '';
	  ...
	  $heading_typography_color = of_get_option('heading_color');
      if ( $heading_typography ) {
        $theme_options_styles .= '
        h1, h2, h3, h4, h5, h6,.page-header a{ 
          font-family: ' . $heading_typography . '; 
          color: ' . $heading_typography_color. '; 
        }';
		
      }
      
      $main_body_typography = of_get_option('main_body_typography');
	  $body_typography_color = of_get_option('body_color');
      if ( $main_body_typography ) {
        $theme_options_styles .= '
        body{ 
          font-family: ' . $main_body_typography . '; 
          color: ' . $body_typography_color . '; 
        }';
      }
      
      $nav_typography = of_get_option('nav_typography');
	  $nav_typography_color = of_get_option('nav_color');
      if ( $main_body_typography ) {
        $theme_options_styles .= '
        .navbar{ 
          font-family: ' . $nav_typography . '; 
          color: ' . $nav_typography_color . '; 
        }';
      }
